// scenarioLoader.ts
// Loads and validates StormSim scenario JSON
...
export function validateScenario(raw: any): void {
...
function validateStandardNode(node: any): void {
...
function validateDocumentationNode(node: any): void {
  ...
  for (const field of node.fields) {
    ...
    if (field.input_type === "single_select") {
      if (!Array.isArray(field.options)) {
        throw new Error(
          `Field ${field.field_id} must have options array`,
        );
      }
    }
  }
...
function validateAuditNode(node: any): void {
